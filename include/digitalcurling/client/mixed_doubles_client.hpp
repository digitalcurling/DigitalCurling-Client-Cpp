...
/// @brief ミックスダブルスカーリング用クライアント
class MixedDoublesClient : public ClientBase {
public:
    /// @brief クライアントを作成する
    ...
    static std::unique_ptr<MixedDoublesClient> Create(
        std::string host,
        std::string id,
...
private:
    std::unique_ptr<IMixedDoublesThinkingEngine> engine_;
